...
impl MainCullPass {
	pub fn new(device: &wgpu::Device, meshes: &MeshesContext, primitives: &PrimitivesContext) -> Self {
	...
	pub fn update_frustum(&self, queue: &wgpu::Queue, camera: &Camera) {
		queue.write_buffer(&self.frustum_buffer, 0, bytemuck::bytes_of(&camera.frustum()));
	}
