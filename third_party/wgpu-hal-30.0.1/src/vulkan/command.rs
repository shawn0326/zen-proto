...
impl crate::CommandEncoder for super::CommandEncoder {
        ...
        T: Iterator<Item = crate::BufferBarrier<'a, super::Buffer>>,
    {
        ...
        for bar in barriers {
            let (src_stage, src_access) =
                conv::map_buffer_usage_to_barrier(bar.usage.from, self.device.features);
            src_stages |= src_stage;
            let (dst_stage, dst_access) =
                conv::map_buffer_usage_to_barrier(bar.usage.to, self.device.features);
        ...
        T: Iterator<Item = crate::TextureBarrier<'a, super::Texture>>,
    {
        ...
        for bar in barriers {
            ...
            let (src_stage, src_access) =
                conv::map_texture_usage_to_barrier(bar.usage.from, self.device.features);
            let src_layout = conv::derive_image_layout(bar.usage.from, bar.texture.format);
            src_stages |= src_stage;
            let (dst_stage, dst_access) =
                conv::map_texture_usage_to_barrier(bar.usage.to, self.device.features);
