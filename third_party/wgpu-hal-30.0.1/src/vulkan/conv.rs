...
pub fn map_texture_usage_to_barrier(
    ...
    let shader_stages = shader_pipeline_stages(features);
...
/// Shader stages that may consume uniform, storage, or sampled resources on this device.
///
/// `VK_EXT_mesh_shader` stage bits are invalid unless the corresponding device features were
/// enabled, so they cannot be added unconditionally. wgpu enables both task and mesh Vulkan
/// features for `EXPERIMENTAL_MESH_SHADER`.
fn shader_pipeline_stages(features: wgt::Features) -> vk::PipelineStageFlags {
    let mut stages = vk::PipelineStageFlags::VERTEX_SHADER
        | vk::PipelineStageFlags::FRAGMENT_SHADER
        | vk::PipelineStageFlags::COMPUTE_SHADER;
    if features.contains(wgt::Features::EXPERIMENTAL_MESH_SHADER) {
        stages |= vk::PipelineStageFlags::TASK_SHADER_EXT | vk::PipelineStageFlags::MESH_SHADER_EXT;
    }
    stages
}
...
mod tests {
    ...
    #[test]
    fn experimental_mesh_devices_include_task_and_mesh_resource_stages() {
        let mesh_features = wgt::Features::EXPERIMENTAL_MESH_SHADER;
        for stages in [
            map_buffer_usage_to_barrier(wgt::BufferUses::STORAGE_READ_ONLY, mesh_features).0,
            map_texture_usage_to_barrier(wgt::TextureUses::RESOURCE, mesh_features).0,
        ] {
            assert!(stages.contains(vk::PipelineStageFlags::TASK_SHADER_EXT));
            assert!(stages.contains(vk::PipelineStageFlags::MESH_SHADER_EXT));
        }

        let ordinary =
            map_buffer_usage_to_barrier(wgt::BufferUses::STORAGE_READ_ONLY, wgt::Features::empty())
                .0;
        assert!(!ordinary.intersects(
