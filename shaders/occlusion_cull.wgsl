...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let clip = params.view_proj * vec4<f32>(world_center, 1.0);
    if (clip.w <= 0.0) {
        visibility_history[instance_index].visible = 1u;
        return;
    }
    ...
    let hiz_depth = textureLoad(hiz_mip0, vec2<i32>(px, py), 0).x;
    ...
    if (object_depth > hiz_depth + bias + slack) {
        visibility_history[instance_index].visible = 0u;
    } else {
        visibility_history[instance_index].visible = 1u;
    }
}
