...
#[derive(Clone, Copy)]
pub(crate) struct FrameTargets<'frame> {
...
pub(crate) struct MeshFrameResources<'frame> {
...
impl<'frame> FrameTargets<'frame> {
    pub(crate) fn register(
        frame: &mut Frame<'frame>,
        surface_texture: &wgpu::Texture,
    ) -> Result<Self, FrameGraphError> {
        Ok(Self {
            color: import_surface_texture(frame, "surface-color", surface_texture)?,
            depth: frame.create_texture(depth_texture_desc(surface_texture.size()))?,
        })
    }
}

impl<'frame> MeshFrameResources<'frame> {
    #[expect(
...
fn import_surface_texture<'frame>(
    frame: &mut Frame<'frame>,
    label: impl Into<String>,
    native: &wgpu::Texture,
) -> Result<Texture<'frame>, FrameGraphError> {
    let handle = frame.import_surface_texture(texture_desc(label, native), Some(native.usage()))?;
    frame.bind_imported_texture(handle, native)?;
    Ok(handle)
}
...
fn depth_texture_desc(size: wgpu::Extent3d) -> TextureDesc {
    TextureDesc {
