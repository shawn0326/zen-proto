...
fn read_only_depth_consumes_defined_contents_without_producing_a_value() {
    ...
    let _ = writer
        .depth_attachment(depth, zen_frame_graph::DepthAttachmentOps::clear_store(1.0))
        .unwrap();
    ...
    let access = full
        ...
        .unwrap();
    assert_eq!(access.mode, zen_frame_graph::AccessMode::Read);
...
fn resolve_target_is_an_overwrite_value_with_render_attachment_usage() {
    ...
    assert_eq!(
        ...
        Some(zen_frame_graph::ResourceUsage::Texture(
...
fn storage_defaults_to_one_mip_and_attachments_reject_multi_mip_views() {
    ...
    let zen_frame_graph::ResourceRange::Texture(regions) = storage_access.range else {
...
fn execution_segments_preserve_external_boundaries() {
    ...
    assert_eq!(
        segments[1].kind,
        zen_frame_graph::ExecutionSegmentKind::ExternalSubmission
    );
}
