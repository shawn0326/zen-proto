...
impl HiZStage {
    ...
    /// Records the complete depth pyramid. Each mip remains a distinct FrameGraph node.
    pub(crate) fn record<'frame>(
        ...
        frame.with_debug_group(group_label, |frame| {
            self.record_depth_to_mip0(
                frame,
                format!("{node_prefix}-depth-to-mip0"),
                targets,
                resources,
                pyramid,
            )?;
            for mip in 1..pyramid.mip_level_count() {
                self.record_mip_to_mip(
                    frame,
                    format!("{node_prefix}-mip{}-to-mip{mip}", mip - 1),
                    resources,
                    pyramid,
                    mip,
                )?;
...
mod tests {
    ...
    fn execute_pyramid(
        ...
        pass.finish_compute(move |mut ctx| {
            ...
            stage.encode_depth_to_mip0(ctx.device, &mut ctx.pass, source, destination, pyramid);
            Ok(())
        })
        .unwrap();
    ...
    fn transient_pyramid_executes_reuses_and_resets_on_resize() {
        ...
        let stage = HiZStage::new(&device);
        let mut graph = FrameGraph::with_device(&device);

        execute_pyramid(&mut graph, &queue, &stage, HiZPyramidDesc::new(13, 7));
        ...
        execute_pyramid(&mut graph, &queue, &stage, HiZPyramidDesc::new(21, 11));
